...
require 'models/most_improved_runs'

describe MostImprovedRuns do

  it_should_behave_like "BasicMostImprovedStats"

  context ".new" do
    Given(:avg_stat) { MostImprovedRuns.new(stats: BattingStat.all, prev_stats: BattingStat.all) }
    Then { avg_stat.should respond_to(:total_runs) }
    Then { avg_stat.should respond_to(:prev_total_runs) }
    Then { avg_stat.should respond_to(:total_runs_diff) }
  end

  describe "with BattingStat datasets" do
    before(:all) do
      create_basic_data(MostImprovedRuns.to_s, '2011')
      create_basic_prev_data(MostImprovedRuns.to_s, '2010')
    end
    after(:all) do
    ...
    basic_givens(MostImprovedRuns.to_s)
